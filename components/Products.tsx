...
function TiltCard({ product, index }: { product: Product; index: number }) {
  ...
  return (
    <Reveal index={index % 3}>
      ...
      >
        {/* Hover glow that follows the tilt */}
        ...
        <div className="mt-7 flex items-center gap-2 text-sm font-medium text-gold opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          Quick view
  ...
  return (
    <section id="products" className="container-px py-28 md:py-36">
      ...
      <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {visible.map((p, i) => (
          <TiltCard key={p.title} product={p} index={i} />
        ))}
      </div>
    </section>
  );
